...
**Audition loop.** Under the keyboard, a 16-step loop plays while you edit, so a change is heard as it is made rather than remembered across a gap. Each step has a note, a velocity and an optional hold that extends the note before it; tempo, step length and gate are set above the grid. The presets are diagnostic rather than musical: a velocity ramp for touch response, a four-octave sweep for keyboard level and rate scaling, fast repeats for envelope retriggering and clicks, a long note and rest for the sustain stage and release tail, and a held chord for detune beating. A level meter beside the play button shows where the patch is against the point SpaceAge and Dexed clip. The transport runs inside the audio engine, so it keeps time while the designer is measuring, and the pattern is never saved into a patch.

**Chord Lab.** A honeycomb beside the loop, one hexagon per degree of the key: pick chords, build a progression of up to eight, and they land on the steps ready to hear. Because it is one cell per degree rather than a circle of fifths, it says something true about every scale OWL knows - seven hexagons for the modes, five for a pentatonic, eight for the diminished scales - and every chord carries a note name as well as its roman numeral - C Phrygian Dominant reads C, D flat, E dim, Fm, G dim, A flat aug, B flat m - spelled the way its key spells it, so F sharp major has an F sharp and D flat major has a D flat. Quality, inversion, voicing and register are per chord, and the selected chord's own hexagon carries two of them: four of its six edges set the inversion and two discs set the octave, the way SpaceAge's numeral pad does. The panel beside the honeycomb is a second view of the same two fields, never a competing one. The point of it in a patch designer is the measurement underneath: every chord is rendered through the current patch and reported as what the mix peaks at, because four stacked carriers clip where one does not, and a patch that measures clean on every single note can still be unusable under a four-note voicing. The musical model is a port of the chord engine in SpaceAge 80s FM, held to that engine's own test cases. The progression can be written out as a Standard MIDI File from the MIDI panel: the chords only, at the loop's tempo, held exactly as long as you hear them. The export is a re-encoding of the same `stepEvents` the loop plays from, so the file and what you heard cannot disagree. A **Tutorial** button beside it runs a guided tour on the real panel — it highlights each control, builds a progression, runs the measurement so you hear and see the result, and puts your loop back exactly as it found it when it finishes. The same steps are written out at [tutorial.html](https://immortalchorus.github.io/owl-operator-waveform-lab/tutorial.html) to read in one go; both are rendered from one list of records, so they cannot disagree.

**Monitoring level.** A VOL control beside the keyboard sets how loud the preview is, from +6 dB down to silence, always opening at -6 dB so the first note is never painful and never a surprise - it is the one preference OWL does not remember between visits, because a level set for headphones at midnight should not still be set that way on speakers in the morning. It sits at the end of the chain, after the peak meter is read, so turning it down cannot hide a patch that would clip in SpaceAge or Dexed, and it never reaches a file.
